...
if image is None:
    ...
    def init_population(pop_size):
        ...
        for _ in range(pop_size):
            gene = [random.randint(0, 255) for _ in range(1)]
    ...
    def crossover(gene1, gene2):
        child1 = [(gene1[0] + gene2[0]) // 2]
        child2 = [(gene1[0] + gene2[0]) // 2]
        return child1, child2

    def mutate(gene, mutation_rate):
        if random.random() < mutation_rate:
            gene[0] = random.randint(0, 255)
        return gene

    def gene_expression_algorithm(image, pop_size=20, generations=50, mutation_rate=0.05):
        population = init_population(pop_size)
        best_threshold = None
        best_fitness = -1

        for generation in range(generations):
            fitness_values = [fitness(gene_expression(individual), image) for individual in population]
            ...
            if current_best_fitness > best_fitness:
                ...
                best_threshold = gene_expression(best_individual)
    ...
    best_threshold = gene_expression_algorithm(image)
